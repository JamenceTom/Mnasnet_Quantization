...
class MnasNet(nn.Module):
    def __init__(self, inchannel, dropout_p=0.2, model_n='mnasnet-a1', num_classes=1000, training=True):
        super(MnasNet,self).__init__()
        self.p = dropout_p
        self.training = training

        if model_n == "mnasnet-a1":
            self._mnasnet_a1(inchannel)
            self.model = nn.Sequential(*self.blocks)
            self.logits = nn.Linear(320, num_classes)
        elif model_n == "mnasnet-b1":
